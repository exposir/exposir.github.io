	...
	return (
		...
		>
			<div sx={{ variant: `styles.waves.${variant}.Sticker` }}>
				{prev && (
					<div sx={sx} style={{ opacity: Math.max(0, currentStep - progress) }} key={currentStep - 1}>
						{prev}
					</div>
				)}
				<div sx={sx} style={{ opacity: 1 - Math.abs(currentStep - progress) }} key={currentStep}>
					{curr}
				</div>
				{next && (
					<div sx={sx} style={{ opacity: Math.max(0, progress - currentStep) }} key={currentStep + 1}>
						{next}
					</div>
				)}
			</div>
...
function ImageWave(props) {
	...
	return <Wave columnComponents={[ImageSticker, Scroller]} childrenToStepColumns={childrenToColumns} {...props} />;
}
